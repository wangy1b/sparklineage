package com.wyb.spark.lineage.LineageCapture.flow
    ...
    def transformRecur(json: JValue): List[GraphElement] = {
      getOperation(json) match {
        ...
        case "insertIntoHiveTable" =>
          val id: String = randomString
          val node =
            Node(id, "insertIntoHiveTable", List((json \ "output").extract[String]))
          val childGraph: List[GraphElement] =
            transformRecur(json \ "child")
          val childId = childGraph.head.id
          val relation = Relation("root", "insertIntoHive", childId, id)
          node :: relation :: childGraph

        case "aggregate" =>
          val id: String = randomString
          val node =
            Node(id, "aggregate", Nil)
          val childGraph: List[GraphElement] =
            transformRecur(json \ "child")
          val childId = childGraph.head.id
          val relation = Relation(id, "aggregate", childId, id)
          node :: relation :: childGraph
