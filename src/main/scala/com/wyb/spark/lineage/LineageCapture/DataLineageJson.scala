...
import org.apache.spark.sql.catalyst.expressions.{Attribute, AttributeSet, BinaryOperator, Expression, Literal, NamedExpression, SortOrder}
...
class DataLineageJson extends DataLineageLogicalPlanVisitor[JValue] {
  ...
  // add wyb
  override def visitInsertIntoHiveTable(insertIntoHiveTable: InsertIntoHiveTable): JValue = {
    JObject(
      JField("op", JString("insertIntoHiveTable")) ::
          JField(
            "output",
            JString(insertIntoHiveTable.table.location.toString)
          ) ::
          JField(
            "format",
            JString(insertIntoHiveTable.table.storage.outputFormat.get.toString)
          ) ::
          JField(
            "mode",
            JString("insertIntoHive")
          ) ::
          JField("child", visit(insertIntoHiveTable.query)) ::
          Nil
    )
  }
  // add wyb
  override def visitInsertIntoTable(insertIntoTable: InsertIntoTable): JValue = {
    JObject(
      JField("op", JString("insertIntoTable")) ::
          JField(
            "output",
            JString(insertIntoTable.output.toString)
          ) ::
          JField(
            "format",
            JString("table")
          ) ::
          JField("child", visit(insertIntoTable.query)) ::
          Nil
    )
  }
