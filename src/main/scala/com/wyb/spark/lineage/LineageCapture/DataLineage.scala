...
trait DataLineageLogicalPlanVisitor[T] {
  def visit(p: LogicalPlan): T = p match {
    ...
    // add wyb
    case p: InsertIntoTable => visitInsertIntoTable(p)
    // add wyb
    case p: InsertIntoHiveTable => visitInsertIntoHiveTable(p)
  ...
  def visitInsertIntoTable(insertIntoTable: InsertIntoTable): T
  def visitInsertIntoHiveTable(insertIntoHiveTable: InsertIntoHiveTable): T
