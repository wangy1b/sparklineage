...
import com.wyb.spark.lineage.linkis.metadata.CSColumn
import com.wyb.spark.lineage.linkis.{SparkHiveObject => HPO}
...
import com.wyb.spark.lineage.linkis.utils.ClassUtils._
  ...
  private[this] def parseRunnableCommand(plan: LogicalPlan,
    ...
    plan match {
      ...
      case c: CreateDataSourceTableAsSelectCommand =>
        val columnList = if (null == c.table.schema || c.table.schema.isEmpty) toCSColumnsByColumnName(c.outputColumns.map(x => x.qualifiedName)) else
          toCSColumns(c.table.schema)
      ...
      case c: CreateHiveTableAsSelectCommand =>
        val columnList = if (null == c.tableDesc.schema || c.tableDesc.schema.isEmpty) toCSColumnsByColumnName(c.outputColumns.map(x => x.qualifiedName)) else toCSColumns(c.tableDesc.schema)
